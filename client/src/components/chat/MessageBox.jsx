...
const MessageBox = () => {
  ...
  const onSentMessage = async () => {
    if (IsEmpty(message)) {
        ...
        if(await sentMessageRequest(message, selectedChat._id))
          setMessage('')
    }
}
...
const fetchAllMessage = async () => {
  if (!selectedChat) return
      ...
      // setLoading(true)
      await fetchAllMessagesRequest(selectedChat._id)
      // setLoading(false)
    ...
    <Fragment>
      {
        ...
        <div className="flex flex-col bg-white overflow-hidden flex-grow h-full">
            ...
            <div className="flex flex-none space-x-1 md:space-x-3 items-center">
              ...
              <div className="flex flex-col pl-1 md:pl-0 max-w-[170px] md:max-w-[480px]">
                <span className="font-bold text-black truncate">
                  {
                    getSender(selectedChat?.users, getUserDetails()).firstname + ' ' + getSender(selectedChat?.users, getUserDetails()).lastname
                  }
                </span>
          ...
          <div className="flex flex-col w-full max-h-auto relative bg-black-rgba scroller overflow-x-hidden overflow-y-auto flex-grow">
            <Message messages={allMessages} /*isTyping={isTyping} typingUser={typingUser}*/ />
          ...
          <div className="flex space-x-2 items-center bg-white px-3 lg:px-6 py-2 border">
            ...
            <div className="flex flex-grow items-center space-x-3 lg:space-x-4 ">
              ...
              <svg onClick={onSentMessage} className='cursor-pointer' xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 64 64" style={{ isolation: "isolate" }} id="send"><defs><clipPath id="a"><rect width="64" height="64"></rect></clipPath></defs><g clip-path="url(#a)">
                <path fill="text-gary-700"
